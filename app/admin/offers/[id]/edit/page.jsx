...
const Page = () => {
  ...
  useEffect(() => {
    const fetchOffer = async () => {
      try {
        const response = await fetch(`/api/offersapi/${params.id}`);
        ...
        setFormData({
          ...
          maxRewardCap: data.maxRewardCap || '',
          validCards: data.validCards?.length > 0 ? data.validCards : [{ bank: '', cardName: '' }]
        });
      } catch (err) {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500 mt-2 text-sm">Offer ID: {params.id}</p>
            </div>
            <button
              onClick={() => router.back()}
